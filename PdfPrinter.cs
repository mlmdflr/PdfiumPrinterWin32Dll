...
namespace PdfiumPrinterWin32Dll
{
    public class PdfPrinter
    {
        ...
        public void PrintP(Stream fileStream,string password, short copies = 1, string documentName = null)
        {
            this.Settings.Copies = copies;
            using (var document = PdfDocument.Load(fileStream, password))
            {
                using (var printDocument = document.CreatePrintDocument())
                {
                    Print(printDocument, documentName);
        ...
        public void PrintP(string fileName, string password, short copies = 1, string documentName = null)
        {
            this.Settings.Copies = copies;
            // Now print the PDF document
            using (var document = PdfDocument.Load(fileName, password))
            {
                using (var printDocument = document.CreatePrintDocument())
                {
                    Print(printDocument, documentName);
